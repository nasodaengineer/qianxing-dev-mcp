...
| Patterns `catalog.json` | Lazy-loaded once; `clearPatternsCache()` for tests. |
...
- Fixed number of module-level caches (nodes, patterns, skills, a few markdown docs, knowledge path index). No unbounded Map growth.
- No long-lived timers; compile uses `spawnSync` timeout (no manual `setInterval`).
- Test helpers: `resetNodesCache()`, `resetKnowledgeCaches()`, `clearPatternsCache()`.
