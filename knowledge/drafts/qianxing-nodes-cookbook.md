# Skill Draft: qianxing-nodes-cookbook

## Purpose
千星奇域节点图速查与编排菜谱。覆盖服务器五类节点 + 客户端多图类型差异提示。数据来自官方 CDN 节点百科正文。

## When to use
- 用户询问某节点做什么、有哪些参数、属于执行/事件/查询/运算/流程
- 设计关卡逻辑、技能、造物 AI、过滤器时需要节点选型
- 区分服务器节点图 vs 客户端节点图（角色技能/造物技能/造物状态/决策/操控技能/过滤器）

## Mental model
1. **服务器节点图**：关卡/实体权威逻辑（创建销毁、战斗结算、背包商店、排行成就、UI 控件组切换等）。
2. **客户端节点图**：表现与本地预测向（技能演出、状态机、过滤器）。同类「查询/运算/执行」在不同图类型下节点集合不同。
3. **事件驱动**：事件节点出参 + 执行节点副作用；查询取数；运算变换；流程控制分支/门控。
4. **高级特性**（附录）：泛型引脚、节点图变量、复合节点、信号、字典、结构体、各类日志。

## Server node categories at a glance
- 执行 `mhw66orrrfkm`: 205 节点 — 副作用与状态写入
- 事件 `mhn7ko01v3yw`: 65 节点 — 触发入口
- 查询 `mhwbqlrw655q`: 151 节点 — 只读取数
- 运算 `mhnd4l069tk0`: 62 节点 — 纯计算/变换
- 流程控制 `mhe8yn9bysd6`: 2 节点

## 服务器·执行节点（`mhw66orrrfkm`，205 节点）
...
- **角色技能节点图** — 167 节点条目；分类：其它节点(1)、执行节点(44)、查询节点(73)、流程控制节点(2)、运算节点(47)
- **角色操控技能节点图** — 178 节点条目；分类：其它节点(1)、执行节点(48)、查询节点(80)、流程控制节点(2)、运算节点(47)
- **过滤器节点图** — 119 节点条目；分类：其它节点(2)、查询节点(70)、运算节点(47)
- **造物技能节点图** — 139 节点条目；分类：其它节点(1)、执行节点(38)、查询节点(52)、流程控制节点(1)、运算节点(47)
- **造物状态决策节点图** — 87 节点条目；分类：其它节点(1)、执行节点(1)、查询节点(40)、流程控制节点(2)、运算节点(43)
- **造物状态节点图** — 100 节点条目；分类：其它节点(1)、执行节点(14)、查询节点(40)、流程控制节点(2)、运算节点(43)

## Common recipes (from node inventory)

### Debug
- `打印字符串`（执行）输出日志；配合节点图日志（附录）定位。

### Loop / list
- `有限循环` / `列表迭代循环` + `跳出循环`；列表：`拼接/清除/插入/移除/设置/排序`。

### Entity lifecycle
- `创建实体` / `创建元件` / `创建元件组` → 配置 → `销毁实体` / `移除实体`；显示：`激活/关闭模型显示`、`修改模型颜色和材质`。

### Combat & survival
- `发起攻击`、`恢复/损失/直接恢复生命`；复苏族：`激活复苏点`、`允许/禁止玩家复苏`、`复苏角色` 等。

### Motion
- 基础运动器添加/暂停/恢复/激活/删除；跟随运动器切目标；`创建投射物`；操控运动器进出。

### UI / peripheral hooks from server exec
- 界面：`切换当前界面布局`、`激活控件组库内界面控件组`、`设置界面控件(组)状态`、`播放界面动效控件`、`更新消息队列`、卡牌选择器唤起/关闭。
- 外围：`以整数/浮点数设置玩家排行榜分数`、`设置/增加成就进度计数`、`设置任务计数`、段位分数与计分组、`消耗礼盒`。
- 资源：商店开闭与商品表、装备词条与装卸、背包容量/道具/货币、战利品与掉落物数量。

### Signals / data structures
- `发送信号`；字典增删排序；`修改结构体`。

## Source files
- Index: `nodes/node-index.json`
- Overview: `SUMMARIES/nodes-overview.md`
- Related: `nodes/related/`（节点图基础概念、编辑指引、高级特性）
