# Skill Draft: qianxing-peripheral-systems

## Purpose
覆盖排行榜、竞技段位、成就、关卡结算、奇域礼盒、奇域任务、先行体验、奇匠订阅。
...
- **结算关卡** (六、关卡相关) — 触发关卡结算流程，会按照关卡结算内的逻辑进行局外的逻辑结算
- **随机卡牌选择器选择列表** (三十三、关卡结算) — 将输入的列表进行随机排序
- **设置玩家结算计分板展示数据** (三十三、关卡结算) — 设置玩家结算计分板展示数据，显示在关卡结算后弹出的计分板内。由于该节点涉及了局外功能的显示，故【数据值】和【数据名称】目前仅在手动输入文本的时候支持多语言翻译，若为连线输入，则不支持多语言翻译
- **设置玩家结算排名数值** (三十三、关卡结算) — 设置玩家结算后的排名数值，再按照【关卡设置】-【结算】中的【排名数值比较顺序】的设置来决定最终的排名顺序
- **设置玩家结算成功状态** (三十三、关卡结算) — 设置玩家结算成功状态
- **设置阵营结算排名数值** (三十三、关卡结算) — 设置阵营结算后的排名数值，再按照【关卡设置】-【结算】中的【排名数值比较顺序】的设置来决定最终的排名顺序
- **设置阵营结算成功状态** (三十四、光源组件) — 设置阵营结算成功状态
- **切换造物巡逻模板** (四十二、排行榜) — 造物切换的巡逻模板即刻切换，并按照新的巡逻模板进行移动
- **以整数设置玩家排行榜分数** (四十二、排行榜) — 以整数设置玩家排行榜分数
- **以浮点数设置玩家排行榜分数** (四十三、成就) — 以浮点数设置玩家排行榜分数
- **设置成就进度计数** (四十三、成就) — 设置指定实体上对应成就序号的成就进度计数
- **增加成就进度计数** (四十四、扫描标签) — 变更指定实体上对应成就序号的成就进度计数
- **设置扫描组件的生效扫描标签序号** (四十五、段位) — 将目标实体的扫描标签组件中对应序号的扫描标签设置为生效
- **设置玩家逃跑合法性** (四十五、段位) — 设置指定玩家逃跑的合法性
- **设置玩家段位变化分数** (四十五、段位) — 根据结算状态设置玩家的段位变化分数
- **切换玩家竞技段位生效的计分组** (四十六、实体布设组) — 以计分组的序号切换指定玩家竞技段位生效的计分组
- **设置玩家聊天语音权限** (四十八、奇域礼盒相关) — 设置目标玩家在频道列表中语音聊天的权限
- **消耗礼盒** (四十九、寻路阻挡) — 可以消耗指定玩家的奇域礼盒
- **唤起悬浮交互页** (五十三、关卡任务) — 可以在该玩家界面唤起对应索引的悬浮交互页，并支持初始化页签/单选项视窗数据
- **设置任务计数** (五十三、关卡任务) — 仅超限模式可用
- **增加任务计数** (五十四、操控运动器) — 仅超限模式可用
