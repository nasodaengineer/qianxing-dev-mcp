# Skill Draft: qianxing-resource-systems

## Purpose
道具 / 装备 / 货币 / 背包 / 掉落物 / 商店 的配置与运行时操作。

## Pipeline
1. 定义道具/货币/装备与掉落规则。
2. 挂载背包/商店/战利品组件。
3. 服务器节点增减数量、改表、开商店、装卸装备。
4. UI 展示；礼盒等外围可消费。
...
- **修改结构体** (三十七、商店) — 在选定结构体后，可以修改该结构体的每个参数
- **打开商店** (三十七、商店) — 在游戏运行过程中以玩家实体的视角打开商店
- **关闭商店** (三十七、商店) — 在游戏运行过程中以玩家实体的视角关闭所有已打开的商店
- **设置自定义商店商品出售信息** (三十七、商店) — 设置自定义商店商品出售信息
- **设置背包商店商品出售信息** (三十七、商店) — 设置背包商店商品出售信息
- **设置物品收购表中道具收购信息** (三十七、商店) — 设置物品收购表中道具收购信息
- **向自定义商店出售表中新增商品** (三十七、商店) — 向自定义商店出售表中新增商品，新增成功后出参会生成一个整数型索引作为该商品的标识
- **向背包商店出售表中新增商品** (三十七、商店) — 向背包商店出售表中新增商品
- **向物品收购表中新增物品** (三十七、商店) — 向物品收购表中新增物品
- **从自定义商店出售表中移除商品** (三十七、商店) — 从自定义商店出售表中移除商品
- **从背包商店出售表中移除商品** (三十七、商店) — 从背包商店出售表中移除商品
- **从物品收购表中移除物品** (三十八、装备) — 从物品收购表中移除物品
- **装备添加词条** (三十八、装备) — 对指定装备实例添加一条预先配置好的词条，可以覆写词条的数值
- **移除装备词条** (三十八、装备) — 移除指定装备实例的对应词条
- **设置装备词条值** (三十八、装备) — 设置指定装备实例对应词条上的值
- **装备指定序号添加词条** (三十八、装备) — 对指定装备实例的指定词条序号位置添加预先配置好的词条，可以覆写词条的数值
- **替换装备到指定栏位** (三十八、装备) — 将指定装备替换到目标实体的装备栏对应栏位
- **卸下指定槽位装备** (三十九、道具与背包) — 卸下装备栏指定行列对应的槽位的装备
- **增加背包最大容量** (三十九、道具与背包) — 增加指定背包持有者的背包最大容量
- **增加背包道具数量** (三十九、道具与背包) — 修改背包内指定道具的数量，会在当前值上加上增加值，增加值可以为负数
- **设置背包掉落道具/货币数量** (三十九、道具与背包) — 设置背包掉落道具/货币的类型和数量
- **增加背包货币数量** (三十九、道具与背包) — 修改背包内指定货币的数量，会在当前值上加上增加值，增加值可以为负数
- **设置背包道具掉落内容** (三十九、道具与背包) — 以字典形式设置背包道具掉落内容，并可以设置掉落类型
- **触发战利品掉落** (三十九、道具与背包) — 对掉落者实体触发一次战利品掉落，可设置其掉落类型
- **设置战利品掉落内容** (三十九、道具与背包) — 以字典形式设置掉落者实体上战利品组件中战利品的掉落内容
- **增加掉落物组件道具数量** (三十九、道具与背包) — 修改掉落物元件上掉落物组件内指定道具的数量，会在当前值上加上增加值，增加值可以为负数
- **增加掉落物组件货币数量** (四十、小地图标识组件) — 修改掉落物元件上掉落物组件内指定货币的数量，会在当前值上加上增加值，增加值可以为负数
