# Skill Draft: qianxing-ui-controls

## Purpose
指导在千星奇域中选型、配置、组合界面控件与控件组；回答布局切换、控件状态、动效、输入反馈等问题。

## Architecture
1. **界面布局**：多套布局，执行节点 `切换当前界面布局`。
2. **界面控件组**：可从库激活/移除，或 `设置界面控件(组)状态`。
3. **素材库**：可复用 UI 素材。
4. **单控件资产**：在「资产 / 界面控件」下配置。
...
| 图片源 | 枚举，支持配置静态引用、动态引用、道具、状态、技能、单位状态根据选择不同，影响后续配置参数 |
...
## Related server exec nodes

- **更改玩家当前职业等级** (二十四、界面控件组) — 修改玩家当前职业等级，若超出定义的等级范围则会失效
- **切换当前界面布局** (二十四、界面控件组) — 可以通过布局索引来切换目标玩家当前的界面布局
- **激活控件组库内界面控件组** (二十四、界面控件组) — 可以在目标玩家的界面布局上激活处于界面控件组库内的以自定义模板形式存在的界面控件组
- **设置界面控件(组)状态** (二十四、界面控件组) — 通过界面控件索引来修改目标玩家界面布局内对应界面控件的状态
- **移除控件组库内界面控件组** (二十四、界面控件组) — 可以在目标玩家的界面布局上移除已通过节点【激活控件组库内界面控件组】激活的界面控件组
- **播放界面动效控件** (二十四、界面控件组) — 在玩家实体的界面布局中播放该界面动效控件挂载的特效资产，如果想隐藏或关闭特效可以用【设置界面控件(组)状态】节点。该节点多次执行时特效可以多次播放。
- **更新消息队列** (二十五、技能) — 针对【消息队列】界面控件，支持通过节点图传输需要显示的一组数据，传输后会在指定【消息队列】界面控件中按照输入的图文组模板显示这组数据
- **切换生效的文本气泡** (三十二、卡牌选择器) — 目标实体的文本气泡组件中，会以配置ID对应的文本气泡替换当前生效的文本气泡
- **唤起卡牌选择器** (三十二、卡牌选择器) — 对目标玩家打开提前制作好的卡牌选择器
- **关闭卡牌选择器** (三十二、卡牌选择器) — 关闭指定玩家当前生效的卡牌选择器
- **随机卡牌选择器选择列表** (三十三、关卡结算) — 将输入的列表进行随机排序
- **激活/关闭寻路阻挡** (五十、光标碰撞盒组件) — 可以修改目标实体的寻路阻挡组件中对应序号的寻路阻挡是否激活
- **激活/关闭光标碰撞盒** (五十一、造物预设状态) — 可以修改目标实体的点击碰撞组件中对应序号的点击碰撞盒是否激活
- **设置复杂造物的预设状态值** (五十二、悬浮交互页) — 可以设置复杂造物的指定预设状态索引的预设状态值
- **关闭悬浮交互页** (五十二、悬浮交互页) — 可以关闭该玩家实体对应索引的悬浮交互页
- **更新悬浮交互页列表数据** (五十二、悬浮交互页) — 以列表显示项入参的整数列表对应的列表项，覆盖列表索引对应的页签/单选项视窗的当前显示项
- **唤起悬浮交互页** (五十三、关卡任务) — 可以在该玩家界面唤起对应索引的悬浮交互页，并支持初始化页签/单选项视窗数据
- **设置玩家激活操控运动器** (五十五、光标) — 设置玩家激活操控运动器。若实体列表为空，则清空玩家所有激活的运动器。
- **设置玩家光标是否常驻** (五十五、光标) — 设置指定玩家的光标是否常驻显示
- **设置玩家光标点击可选取目标** (五十五、光标) — 设置指定玩家光标点击时可选取的目标层级与最多选取数量
- **设置玩家光标是否穿透UI控件** (五十五、光标) — 设置指定玩家光标点击是否能够穿透UI控件，对UI下方的对象生效

## Sources
- See `/workspace/qianxing-deep/` area folders + SUMMARIES
